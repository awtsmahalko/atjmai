...
<body class="blue-skin">
				...
				<div class="container">
					<div class="row align-items-center">
						<div class="col-lg-12 col-md-12 text-center">
							<p class="mb-0">© 2023 NONESCOST Alumni Tracker with Job Matching using AI Integration</p>
						</div>
					</div>
				</div>
			</div>
	...
	<script>
		...
		$("#frmRegisterEmployee").submit(function(e) {
			e.preventDefault();
			$("#btn_register_employer").prop('disabled', true);
			$("#btn_register_employer").html('Registering...');
			$.post("controller/web.php?uri=signup", $("#frmRegisterEmployee").serialize(), function(data, status) {
				// var res = JSON.parse(data);
				if (data == 1) {
					// SUCCESS
					$("#response_register_employer").html('<div class="alert alert-primary" role="alert">Successfully registered! <br> <b> Page will redirect in <span id="countdown">5</span> seconds!</div>');
					countDown();
				} else if (data == 2) {
					// EMAIL ALREADY TAKEN
					$("#response_register_employer").html('<div class="alert alert-danger" role="alert">Email is already taken!</div>');
				} else if (data == -2) {
					// PASSWORD DOES NOT MATCH
					$("#response_register_employer").html('<div class="alert alert-danger" role="alert">Password does not match!</div>');
				} else if (data == -1) {
					// EXPIRED CSRF TOKEN
					$("#response_register_employer").html('<div class="alert alert-danger" role="alert">Token already expired!<br> <b> Page will redirect in <span id="countdown">5</span> seconds!</div>');
					countDown();
				} else {
					$("#response_register_employer").html('<div class="alert alert-danger" role="alert">' + data + '</div>');
				}
				$("#btn_register_employer").prop('disabled', false);
